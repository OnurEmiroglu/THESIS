...
Karar #33 (misspec deney tasarimi), #34 (strong variant skip), #35 (oracle >=90%) ekle,
ozet tabloyu guncelle."""
...
def main():
    ...
    _add_decision_table(
        ...
        rows_data=[
            ...
            (
                "oracle_full sigma_only'yi geçememiştir (p=0.301). Bu bulgu "
    ...
    # ── Özet tablosu güncelle: 12 -> 15 ──
    for p in doc.paragraphs:
        if "Kritik 12 Karar" in p.text:
            for run in p.runs:
                if "12" in run.text:
                    run.text = run.text.replace("En Kritik 12 Karar", "En Kritik 15 Karar")

    # Add rows #13, #14, #15 to summary table (last table)
    ...
    _add_summary_row(
        sum_tbl, "15",
        "Oracle deneyi >=90% dedektör hedefini karşılıyor — %100 doğruluk bile sigma_only'yi geçemedi (p=0.301)",
        "WP5",
    )
