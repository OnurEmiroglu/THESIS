"""thesis_16.docx temel alınarak thesis_17.docx üretir.
Rejime koşullu envanter cezası deneyi (Section 4.7) eklendi."""
...
p.add_run("Financial Engineering MSc Programı")
p = dst.add_paragraph(); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
p.add_run("Mart 2026 — Sürüm 17")

# ================================================================
# 1. ÖZET — from thesis_14
# ================================================================
...
add_normal(dst, (
    ...
    "Buna karşın, rejim bilgisinin PPO performansına net bir katkı sağladığı hipotezi istatistiksel olarak "
    "desteklenmemiştir."
))

add_normal(dst, (
    "Ana Bulgu (Ablasyon): oracle_full ajanı (sigma + regime_true), mükemmel rejim bilgisine sahip olmasına "
    "karşın ppo_sigma_only'yi (Sharpe: 0.753 vs 0.722) geçememiştir. oracle_full ile combined arasındaki fark "
    "istatistiksel olarak anlamsızdır (p = 0.301). Bu sonuç, rejim etiketinin — gerçek ya da tahmini — "
    "sigma_hat'in zaten taşıdığı bilginin üzerine anlamlı katkı sağlamadığı hipoteziyle güçlü biçimde tutarlıdır."
))
...
add_normal(dst, (
    ...
    "(tüm dedektörler için Sharpe bazlı p > 0.08)."
    " Ek olarak yürütülen rejime koşullu envanter cezası deneyi (ηH = 5×ηL) de "
    "sigma_only varyantını geçememiştir (p = 0.0016), signal redundancy yorumunu "
    "ödül tasarımı boyutunda da desteklemektedir."
))

add_normal(dst, (
    "Anahtar kelimeler: piyasa yapıcılığı, pekiştirmeli öğrenme, PPO, volatilite rejimleri, "
    "Avellaneda-Stoikov, ablasyon, oracle agent, sinyal yedekliliği"
))

# ================================================================
# SEMBOLLER VE KISALTMALAR (YENİ — thesis_13.pdf'ten)
# ================================================================
...
# ================================================================
# TERİMLER SÖZLÜĞÜ (YENİ — thesis_13.pdf'ten)
# ================================================================
...
# ================================================================
# 3. TEORİ VE METODOLOJİ — genişletilmiş (thesis_13.pdf'ten)
# ================================================================
...
# 3.7 Ablasyon Tasarımı (from thesis_14)
...
# 3.8 PPO Eğitimi (with hyperparameter table)
...
add_normal(dst, (
    ...
    "PPO varyantları inv_p99 ≈ 2 tick düzeyinde çalışırken, AS ve naif stratejiler sırasıyla 29.95 ve 21.20 "
...
add_normal(dst, (
    ...
    "Wilcoxon işaretli sıra testi: p = 0.261 (anlamlı değil). Eşleştirilmiş t-testi (equity): p = 0.023."
))
...
# Üç hipotez (thesis_13.pdf sayfa 12)
...
add_normal(dst, (
    ...
    "referansıdır. Sol: Sharpe (Wilcoxon p=0.261). Sağ: Final equity (paired t-test p=0.023). "
...
# 4.2 Pure Ablation (from thesis_14)
...
# 4.3 İstatistiksel Testler (from thesis_14)
add_heading(dst, "4.3 İstatistiksel Testler (Ablasyon)", level=2)
add_normal(dst, (
    "Tablo 3, 10 karşılaştırmanın eşleştirilmiş t-testi sonuçlarını göstermektedir "
    "(n=20 seed, Bonferroni düzeltmesi: α = 0.01/10 = 0.001)."
))
...
# 4.4 Eylem Analizi (genişletilmiş — thesis_13.pdf sayfa 13-14)
...
add_normal(dst, (
    "Şekil 3. Rejim bazında eylem dağılımı (20 tohum, ortalama ± std). Üst: half-spread h. Alt: skew m. "
...
# 4.5 Rejim Bazlı Performans Analizi (genişletilmiş — thesis_13.pdf sayfa 14)
...
add_normal(dst, (
    "Şekil 4. Volatilite rejimine göre Sharpe oranı (20 tohum, ortalama ± std). PPO'nun üstünlüğünün "
...
# 4.6 Detector Robustness (from thesis_14)
...
add_normal(dst, (
    "Şekil 5. Dedektör bazında ortalama Sharpe (20 tohum). PPO-blind Sharpe değeri tüm dedektörlerde "
    "0.753 olarak sabit kalmakta; PPO-aware ise dedektör seçiminden bağımsız olarak ~0.717-0.719 "
    "düzeyinde seyretmektedir."
))

# ================================================================
# 5. SONUÇ — from thesis_14
# ================================================================
...
add_normal(dst, (
    ...
    "desteklenmemiştir. Rejim bilgisinin ilave edilmesi istatistiksel olarak anlamlı performans artışı "
    "sağlamamaktadır (Sharpe bazlı Wilcoxon p = 0.261, equity bazlı t-test p = 0.023). Bu null sonuç, "
    "gözlem uzayında hali hazırda bulunan σ̂_t'nin rejim bilgisini örtük olarak içermesiyle açıklanmaktadır."
))

add_normal(dst, (
    "Bulgu 3 — Pure Ablation (Sinyal Yedekliliği Kanıtı): ppo_sigma_only en yüksek performansı elde "
    "etmiştir — oracle_full'ı bile geçmektedir. oracle_full ile combined arasındaki fark istatistiksel "
    "olarak anlamsızdır (p = 0.301 Sharpe, p = 0.360 equity)."
))
...
add_normal(dst, (
    ...
    "yerine ηL = 0.0005, ηM = 0.001, ηH = 0.0025 konfigürasyonu kullanılmış; üç varyant "
...
dst.add_paragraph("(2) Model yanlış belirleme (model misspecification) benchmark'ı — AS parametrelerinin (A, k) rejime bağlı kılınması.", style='List Paragraph')
...
# ================================================================
# APPENDIX B — Source Code File Index (thesis_13.pdf sayfa 18)
# ================================================================
...
app_r = [
    ...
    ["src/wp5/stats_detector_robustness.py", "İstatistiksel testler (Wilcoxon, t-test, ANOVA)"],
...
# SAVE
# ================================================================
dst.save("manuscript/thesis_17.docx")
print("thesis_17.docx saved successfully.")
