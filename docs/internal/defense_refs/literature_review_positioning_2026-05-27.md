...
Scope: thesis literature review and revised thesis draft direction only. This note keeps the contribution defense-safe and bounded to the controlled synthetic high-frequency market-making environment.
...
RL and DRL market-making studies motivate learning quote placement from simulated or model-driven environments rather than relying only on analytical approximations. This literature is directly relevant because it studies learned policies, state representations, inventory penalties, and robustness of market-making behavior.

The thesis should position itself as narrower than general RL market making. It does not claim to introduce the best market-making RL algorithm. Its contribution is an incremental signal-representation test inside a controlled synthetic HFMM setup.
...
This thesis does not claim that volatility regimes are irrelevant. It tests whether an explicit categorical volatility-regime label provides incremental value once a continuous realized-volatility proxy, `sigma_hat`, is already observed by the PPO policy.

In the tested synthetic high-frequency market-making environment, the evidence is consistent with signal redundancy: explicit labels do not provide robust additional performance beyond the continuous volatility signal.

## 4. Signal/State Representation and Redundancy

The central interpretation is signal redundancy. If `sigma_hat` already carries the volatility information needed for quote adaptation, a categorical label derived from related volatility information may add little or no usable incremental state information in the tested synthetic environment.

Defense-safe phrasing is: the results are consistent with signal redundancy. The thesis should avoid claiming a proven internal mechanism. The experiments show observed performance patterns under specified synthetic conditions; they do not prove exactly how the PPO policy internally uses or ignores each observation channel.
...
- "in the tested synthetic environment"
- "no robust incremental value"
- "consistent with signal redundancy"
- "explicit labels do not improve performance once `sigma_hat` is already observed"
- "does not imply regimes are irrelevant"
- "does not prove the internal PPO mechanism"
- "the evidence is from a controlled synthetic HFMM setting"
- "the thesis evaluates incremental signal value, not deployment viability"
...
- "works outside the controlled thesis environment"
