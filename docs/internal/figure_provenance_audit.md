...
## WP2 — Volatilite Rejim Kalibrasyonu

- Source CSV: `wp2_synth.csv` — run-specific canonical at `ctx.run_dir/wp2_synth.csv` (Lane C standardization, commit `e0c47c5`); convenience snapshot at `data/processed/wp2_synth.csv`.
- Generated by: `src/wp2/job_w2_synth.py` (synthetic regime generation, rolling-RV detection, and threshold calibration via `src/wp2/synth_regime.py`).
- Thesis location: Chapter 2 calibration table — **flag for table-number verification**. The canonical "Tablo N" headings in `manuscript/thesis_29.pdf` begin at Chapter 4 (Tablo 1 = WP5 main); WP2 calibration numbers appear in the regime-detection methodology narrative of Chapter 2/3 rather than under a numbered table heading.
- Canonical calibration result (per `docs/wp2/wp2_notes.md`): detection accuracy 32.62% → 60.70% after applying sticky transition matrix + `rv_window = 50` + `warmup_steps = 1000`; threshold gap 0.0064 → 0.0320 (≈5×).
- Note: table/narrative-only evidence; no thesis figure renders directly from this CSV.

## WP5.5 — Offline Calibration Audit

- Source CSVs:
  - `metrics_signal_audit.csv` — produced by `src/wp5_5/job_w55_audit.py` (writes `ctx.run_dir/metrics_signal_audit.csv`).
  - `runtime_summary.csv` and `runtime_decision.md` — produced by `src/wp5_5/job_w55_runtime.py` (writes both under `ctx.run_dir/`).
- Generated by: `src/wp5_5/job_w55_audit.py`, `src/wp5_5/job_w55_runtime.py`, `src/wp5_5/job_w55_calibration.py`; shared helpers in `src/wp5_5/signal_audit.py` and `src/wp5_5/signal_degradation.py`.
- Thesis location: **Tablo 8** — "WP5.5 Sinyal Denetim Metrikleri (kanonik operating point, tohum = 123)" (`manuscript/thesis_29.pdf`, around line 899 of the pdftotext extract).
- Note: table-only / text-only support evidence; no standalone thesis figure renders directly from these CSVs.
